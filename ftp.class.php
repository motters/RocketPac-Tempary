...
class ftp extends rocketpack{
	...
	public function ftpConnect(){
		...
		$this->storeFtpConnection = ftp_connect($this->host, $this->port);
		
		if($this->storeFtpConnection){
			//Now we need to login	
			if($this->authType == 'anonymous'){
				//Here we login via a anonymous auth
				$stausLogin = ftp_login($this->storeFtpConnection, 'anonymous', '');
			}else{
				//Here we login with the username and password
				$stausLogin = ftp_login($this->storeFtpConnection, $this->username, $this->password);
			}
			
			//Here we check that they login successully
			if($stausLogin){
				if(ftp_pasv($this->storeFtpConnection, $this->passiveMode)){
	...
	public function makeDirectory($makeDirectory){
		...
		if(ftp_mkdir($this->storeFtpConnection, $makeDirectory)){
			return true;
		}else{
	...
	public function setCurrentDirectory($location){
		...
		if (ftp_chdir($this->storeFtpConnection, $location)) { 
			return true;
		}else{
	...
	public function listFilesInDirectory($directory='.', $additionPrams = '-la'){
		$fileArray = ftp_nlist($this->storeFtpConnection, $additionPrams . ' ' . $directory);
		if($fileArray){
			return $fileArray;
		}else{
			return false;	
	...
	public function ftpDeleteFile($file){
		...
		$ftpDelete = ftp_delete($this->storeFtpConnection, $file);
	 ...
	 *@Author Sam Mottley
	 */
	public function ftpDeleteEmptyFolder($folder){
		//Attempt to delete Folder
		$ftpDelete = ftp_rmdir($this->storeFtpConnection, $folder);
	 ...
	 *@Author Sam Mottley
	 */
	public function ftpChmodItem($file, $permissions){
		...
		if(strstr($file, '/')){
			...
			if($this->setCurrentDirectory($pathInfo['dirname'])){
				...
				$ftpChmod = ftp_chmod($this->storeFtpConnection, $permissions, $pathInfo['basename']);
			}
			$this->setCurrentDirectory($currentLocation);
		}else{
			//Attempt to chmod file
			$ftpChmod = ftp_chmod($this->storeFtpConnection, $permissions, $file);	
	...
	public function sysType(){
		...
		$systemType = ftp_systype($this->storeFtpConnection);
	...
	public function currentDirectory(){
		...
		$currentDirectory = ftp_pwd($this->storeFtpConnection);	
	...
	public function parentDirectory(){
		...
		$parentDirectory = ftp_cdup($this->storeFtpConnection);
	...
	public function getFileSize($file){
		...
		$getFileSize = ftp_size($this->storeFtpConnection, $file); 
	...
	public function lastModified($file){
		$lastModified = ftp_mdtm($this->storeFtpConnection, $file);
	...
	public function uploadFileContentToExsistingFile($fileLocation, $fileServer, $transferMode=FTP_BINARY){	
		//Here we upload the file
		 $uploadFile = ftp_put($this->storeFtpConnection, $fileServer, $fileLocation, $transferMode);
	...
	public function uploadFile($fileLocation, $fileServer, $transferMode=FTP_BINARY){	
		...
		//Here we upload the file
		$uploadFile = ftp_fput($this->storeFtpConnection, $fileServer, $filePointer, $transferMode);
	...
	public function retrieveFile($fileLocation, $fileServer, $transferMode = 'FTP_BINARY'){
		...
		$retieveFile = ftp_fget($this->storeFtpConnection, $filePointer, $fileServer, FTP_BINARY, 0);
